...
author or run a study — that is [the ASS page](index.md).

## Why this unit exists at all

`ass_flow` owns authoring and the Plan. `ass_exec` owns one attempt's durable
record and imports neither this package nor Dask. `ass_run` owns binding and
...
Before it, an `@operation` body was dead code — `ass_flow` kept the function
...
something other than what was authored. See `ass/ONTOLOGY.md` for the fuller
...
`ass_exec`, a bug in readiness or placement belongs in `ass_run`, and a bug in
what the Plan says belongs in `ass_flow`. What belongs here is only the
...
- [`ass/ONTOLOGY.md`](https://github.com/smldis/analog-sim-studies/blob/main/ass/ONTOLOGY.md)
  ...
  real farm.
- `ass-flow/docs/architecture.md` — the authoring model and the Plan IR,
  including the experimental Dask lowering `ass.visualize` draws from.
- `ass-exec/docs/index.md` — attempt identity, the journal, and the reuse
  decision this package defers to.
- `ass-run/docs/index.md` — the two kernels, binding rules, and `Site`.
