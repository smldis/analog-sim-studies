...
The status tables describe current capabilities. Dated decision narratives below
preserve the argument and assumptions at the time; their present-tense wording
is not a substitute for the current tables or the units' maintained guides.
...
| Declarative source handoff: artifact contract and address | `hedloom-flow` | Current Plan IR is schema 4, including execution mode and output identity. |
...
| Pooled LSF via `dask_jobqueue.LSFCluster` | `hedloom-run` | Implemented by `hedloom_run.pooled.LSFPooledTransport` and Session-owned pools. Commands run on reusable farm workers while attempt accounting stays on the submit host. `hedloom/tests/test_pooled_placement.py` exercises all-pooled and mixed local/pooled Plans against the fake farm. Sequential pooled execution refuses. This is not evidence of real-farm pooling. |
...
| Component boundary and name | One rebuilt "Hedloom Flow" | Split into `hedloom-flow` (planning), `hedloom-exec` (attempts), and `hedloom-run` (binding a run, with readiness now Dask's), coupled through the Plan document. The enclosing `hedloom` facade now owns the operator-facing author/submit/result/history join; the child packages retain their narrower contracts. |
...
| ~~Unified flow-level `submit()`~~ **Built as the `hedloom` unit (2026-08-04).** | One file authors and runs a study. The operation body is the implementation and receives `out`, a workspace of its declared file outputs; returning `shell(...)` makes it a launcher whose command runs at the invocation's placement; `sweep(points, key=...)` keys every call inside the loop; the Plan (now schema 4) records each operation's entry point and a fingerprint of its source, so an edited body can invalidate the work it produced; when source is unavailable, version declarations still matter; declared output bindings live on the operation, retiring the run-time `outputs=` dict; `Site` holds placements, roots, address spaces and threads. Evidence: `studies/rc_corners.py` covers local ngspice, identity and reuse, and `examples/farm_smoke.py` has reached a real farm through the sequential kernel. `hedloom-exec` still imports neither `hedloom_flow` nor Dask. **Met 2026-08-16 against the fake farm:** the smoke tests run the graph kernel through `bsub -I` and `BoundTransport`. The current example takes a Site profile; capacity selects the graph path, and there is no `--dask` CLI flag. Still unmet: that same crossing against a real farm. |
| One-scheduler mixed topology (labelled local, direct-gateway, pooled workers) | The useful mixed-placement capability is implemented using a local readiness `SpecCluster` plus separate `LSFCluster` pools, not one scheduler. The fake-farm mixed test proves local and pooled work in one Plan; it does not establish the proposed single-scheduler topology or real-farm mixed behavior. |
...
| ~~Real direct-LSF smoke test (evidence ladder 6)~~ **Built through the sequential kernel.** | `examples/farm_smoke.py` passed on the real farm, validating `bsub -I`, artifact chaining, failure recording and reuse. It did not exercise Dask, concurrency, or queue-wait observability. **Closed against the fake farm 2026-08-16:** graph-kernel smoke coverage crosses that boundary, and the test asserts that `max_jobs = 2` holds four independent jobs to two in flight — removing the placement annotation from the kernel fails it with `assert 4 == 2`, so the cap is demonstrated rather than assumed. Worth recording from building it: Dask keeps a consumer on its producer's worker for locality, which held the *chained* sweep to two on its own, so a test written over the chained jobs would have passed against the broken kernel. Independent jobs are what give it teeth. |
| Plugins and declarative flow configuration | A concrete multi-repository or non-Python authoring need. |
| **NFS synchronization across submit hosts** | Partly measured, still deployment-dependent. The current pool sends commands to farm workers while journals stay on the submit host; it does not introduce farm-node journal writers. The recorded NFSv3 `local_lock=none`/`nlockmgr` check establishes exclusion between two open descriptions in one process. Cross-host exclusion and attribute-cache visibility remain unmeasured. See [the maintained evidence and probes](../../hedloom/docs/guide/first-farm-run.md). Recheck before multiple submit hosts share a root; local/fake-farm tests cannot establish that deployment contract. |
| Retry policy for indeterminate transport failures | Still deferred. Exec owns the distinction between refused submission and indeterminate transport failure. Current durable execution handles explicitly refuse automatic worker replay before a second entry into Exec (`hedloom_run.execution.ExecutionHandle.enter`); content-addressed identity alone is not permission to enable Dask retries. Trigger to revisit: a workload needing transport recovery, with explicit handling of execution ownership and history. |
| General artifact integrity and provenance | Partly implemented: schema 4 supports full-file content identity for owned and external files, and author-declared identity for filesystem outputs. Default producer identity avoids content hashing. History records candidate inputs separately from the selected try's executed inputs. Directory, stream, and returned-value content identity remain unsupported; borrowed-path revision and lifetime remain the author's responsibility. See [runtime artifact identity](../../hedloom/docs/guide/runtime-artifacts.md). |
...
The following records the decision at that time. Pooling has since been
implemented in `hedloom-run`; it uses a separate pool scheduler and worker-local
client plugin. The old statements that `lsf-pool` refuses and that mixed
composition is only assumed are superseded by the current status tables above.
