...
ASS is the front door: author a study, see what it will do, and run it — from
one import.
...
`@operation` here is `ass_flow`'s decorator, wrapped so the body it already
...
`ass_flow.authoring.ArtifactValue.__bool__` and `__eq__` both raise
...
`local`; the launcher reaching a real LSF job is designed and, at the `ass`
...
to Dask instead (`ass_run.graph.run_plan_graph`); the cluster is always the
...
`.toolchain/venv/bin/python ass/examples/ota_pvt_clean.py [--fresh]` — it needs
...
repeated — this is `ass_exec`'s decision, made against content-addressed
identity, and `ass` neither re-implements nor overrides it. What is folded
...
`ass-flow/docs/architecture.md`. `ass.visualize` gives that lowering the one
...
```python
import ass.visualize as visualize
...
`ass.visualize` is a submodule, not part of `ass`'s top-level `__all__` —
importing it is a deliberate, separate step from importing `ass` itself,
