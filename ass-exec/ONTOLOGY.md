...
It exists because an externally executed job outlives the process that
submitted it. Something whose lifetime is bounded by a live worker, executor,
or client cannot be the authority for such a job. This unit is the durable
...
The unit currently studies one question: whether a durable record plus a
transport that declares its own discoverability is sufficient to make external
execution recoverable, without absorbing any graph scheduling authority. Its
evidence is the two failure injections named by the architecture — acceptance
without receipt, and terminal state without a recorded manifest — reproduced
locally against a fake substrate whose state outlives its caller.

No real batch system is implemented. The only working substrate is in-process
execution, which is the honest degenerate case rather than a simulation of a
remote one.
