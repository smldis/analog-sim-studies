...
| --- | --- | --- | --- | --- |
| `reference.ota_pvt.prepare_run@1` | `base: sidecar-base-directory`, `edits: sidecar-edit-file` | `point_id`, `param_set`, `process`, `vdd_v`, `temp_c` | `prepared-simulation-directory` | Declared plan stub corresponding conceptually to `sidecar-render` and the published resolve/materialize preparation path; no Hedloom Flow adapter currently accepts these bindings. |
