...
## Active development inquiry: rebuild Hedloom Flow
...
Its Hedloom Flow concepts and the evidence produced by subsequent authorized work
are classified in the component-owned
[architecture and research ledger](../children/hedloom-flow/docs/architecture.md). This
...
The result must remain generic within Hedloom. Simulation, CACE integration,
...
Do not build an Hedloom graph scheduler merely to connect these modes. If the
...
plugin behind a new Hedloom workflow engine. Dask should continue to own graph
dependencies, readiness, priorities, retries, ordinary result propagation,
and its diagnostics. Hedloom should add domain-neutral authoring ergonomics,
...
The ordinary Hedloom-facing surface should be compact and hide raw
...
labels may enforce routing, but arbitrary Hedloom metadata has no scheduling effect
unless the Hedloom layer or a deliberate plugin interprets it.
...
all Hedloom inputs, evidence promotion, a workflow server, or a complete durable
