...
from ass_exec.lsf import CommandResult, LSFInteractiveTransport, LSFPooledTransport
...
def test_submission_is_interactive_named_and_walltime_bounded():
    ...
    lsf.submit("ass-abc", BUNDLE)

    argv = runner.calls[0]
    assert argv[0] == "bsub"
    assert "-I" in argv
    assert argv[argv.index("-J") + 1] == "ass-abc"
...
def test_a_bundle_without_a_command_is_refused_before_submission():
    ...
    with pytest.raises(SubmissionRefused):
        lsf.submit("ass-abc", {"arguments": {"value": 1}})
    assert runner.calls == []


def test_successful_exit_becomes_a_published_success(tmp_path):
    lsf, _ = transport()
    journal = AttemptJournal(tmp_path, "ass-abc")
...
def test_discovery_reports_nothing_when_no_job_survives():
    lsf, _ = transport()
    assert lsf.discover("ass-abc") is None
...
def test_cancellation_targets_the_attempt_name():
    lsf, runner = transport()
    lsf.cancel({"identity": "ass-abc"})
    assert runner.calls[-1] == ["bkill", "-J", "ass-abc"]
...
def test_pooled_execution_refuses_rather_than_pretending():
    with pytest.raises(NotImplementedError):
        LSFPooledTransport().submit("ass-abc", BUNDLE)
